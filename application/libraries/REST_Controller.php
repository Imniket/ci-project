...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct() {
        ...
        $api_key = "q1fgdfggfw2e2rt3y5u6i8iug4h58fgh";

        /*         * ***** Value store device table Start ****** 
        $deviceDetail['deviceType'] = $this->input->post('deviceType');
        $deviceDetail['deviceToken'] = $this->input->post('deviceToken');
        $deviceDetail['intUdId'] = $this->input->post('intUdId');
        $deviceDetail['createdOn'] = date('Y-m-d h:i:s');
        $deviceDetail['accessToken'] = $this->input->post('accessToken');
        ...
        if ($accessToken != "" && $userID != "") {
            $this->db->update("tbl_devices", $deviceDetail, array('accessToken' => $accessToken));
            ...
            if (empty($checkAccessToken) || $checkAccessToken['accessToken'] != $accessToken) {
                //$error = array('status' => 'Failed', 'errorcode' => '-13', 'msg' => 'Please login again.');
